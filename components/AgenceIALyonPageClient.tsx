  ...
  return (
    <section style={{ padding: '120px 24px 80px', position: 'relative', overflow: 'hidden', borderBottom: '1px solid #e4e4e7' }}>
      <div aria-hidden="true" style={{ position: 'absolute', inset: 0, zIndex: 0, pointerEvents: 'none' }}>
        <div style={{ position: 'absolute', top: '-10%', left: '-5%', width: 600, height: 600, borderRadius: '50%', background: `radial-gradient(circle,${AC}0a 0%,transparent 65%)`, filter: 'blur(80px)', animation: 'lyo-drift 14s ease-in-out infinite' }} />
      </div>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 1160, margin: '0 auto' }}>
        <div className="lyo-hero-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
          <div>
            <nav aria-label="Breadcrumb" style={{ fontSize: 13, color: '#a1a1aa', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', fontWeight: 500 }}>
              ...
              <span style={{ color: '#09090b' }}>Lyon</span>
            </nav>

            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 24 }}>
              {['+150 PME équipées en France', 'Cas client lyonnais validé', 'Présentiel + distanciel'].map((t) => (
          ...
          <div className="lyo-map-col" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            <FranceMapSVG mainCity="Lyon" presentielLabel="Présentiel Lyon" />
          </div>
        </div>
      </div>
    </section>
...
const raisons = [
  ...
  },
  {
    ...
    body: (
      <p style={{ fontSize: 16, color: '#8a8a95', lineHeight: 1.8, margin: 0 }}>
        Notre stack standard est française : Mistral hébergé en France (OVH, Scaleway), infrastructure auto-hébergeable, aucune donnée nominative envoyée à OpenAI ou Anthropic sans accord client explicite. Particulièrement pertinent pour les ETI lyonnaises soumises à des contraintes secteur sensible : <strong style={{ color: '#09090b' }}>biotech et santé à Gerland et Lyonbiopôle</strong>, banque et finance (Crédit Agricole, BPCE), industrie chimique Vallée de la chimie, pharma (Sanofi Pasteur, BioMérieux). Voir <a href="/#souverainete" style={{ color: AC, fontWeight: 700, textDecoration: 'none' }}>Souveraineté</a>.
      </p>
    ),
  },
  {
    ...
    body: (
      <p style={{ fontSize: 16, color: '#8a8a95', lineHeight: 1.8, margin: 0 }}>
        Pas de PowerPoint à 100 000 € en 6 mois. <strong style={{ color: '#09090b' }}>Un agent IA simple est en production sous une semaine</strong> après cadrage signé. Le ROI typique se mesure en moins de 6 mois sur les cas concrets : notre cabinet d'expertise comptable lyonnais signature a doublé sa capacité en 4 mois. +758 agents déjà en production, +5 M€ d'économies cumulées. Voir <a href="/cas-clients/" style={{ color: AC, fontWeight: 700, textDecoration: 'none' }}>nos cas clients</a>.
      </p>
    ),
  },
];
    ...
    <section id="pourquoi" ref={ref} style={{ padding: '96px 24px', background: '#fff', borderTop: '1px solid #e4e4e7' }}>
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        <div style={{ marginBottom: 52, opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(16px)', transition: 'all .5s ease' }}>
          <H2 style={{ marginBottom: 16 }}>Pourquoi travailler avec Althoce pour votre projet IA à Lyon</H2>
        ...
        <div style={{ marginTop: 48, borderRadius: 20, background: '#09090b', border: '1px solid #1e1e1e', overflow: 'hidden', opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(16px)', transition: 'all .5s .5s ease' }}>
          <div style={{ padding: '14px 28px', borderBottom: '1px solid #1a1a1a', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <p style={{ fontSize: 12, fontWeight: 800, color: '#8a8a95', textTransform: 'uppercase', letterSpacing: '.12em', margin: 0 }}>Présentiel ou distanciel : c'est vous qui décidez selon le projet</p>
          </div>
          <div style={{ padding: '24px 28px' }}>
            <p style={{ fontSize: 16, color: '#a1a1aa', lineHeight: 1.8, margin: '0 0 16px' }}>
              Nos clients lyonnais se répartissent entre présentiel intégral, hybride et tout-distanciel. <strong style={{ color: '#e4e4e7' }}>Nos délais et notre qualité sont identiques quelle que soit la modalité.</strong> Le présentiel reste pertinent pour le cadrage et les ateliers stratégiques. Le distanciel pour le build (60 à 70 % du projet) gagne du temps à tout le monde.
            </p>
...
function MetierMockup({ index }: { index: number }) {
  const mockups = [
    ...
    <div style={{ background: '#fff', borderRadius: 14, padding: 20, boxShadow: '0 4px 28px rgba(0,0,0,.09)', border: '1px solid #e4e4e7' }}>
      <div style={{ fontSize: 12, fontWeight: 800, color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 14 }}>Cabinet comptable · Lyon</div>
      {[{ l: '×2 capacité à effectif constant' }, { l: '−60 % temps de production' }, { l: '0 départ équipe après déploiement' }, { l: '+80 clients absorbés en 4 mois' }].map((r, i) => (
    ...
    // Ops
    <div style={{ background: '#fff', borderRadius: 14, padding: 20, boxShadow: '0 4px 28px rgba(0,0,0,.09)', border: '1px solid #e4e4e7' }}>
      <div style={{ fontSize: 12, fontWeight: 800, color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 14 }}>ETI industrielle · Auvergne-Rhône-Alpes</div>
      {[{ l: 'Volume traité', v: '×3' }, { l: 'Turnover poste assistant ops', v: '80 % → 0 %' }, { l: 'Mails entrants traités / jour', v: '+210 %' }, { l: 'Embauches supplémentaires', v: '0' }].map((r, i) => (
    ...
    <div style={{ background: '#09090b', borderRadius: 14, padding: 20, boxShadow: '0 4px 28px rgba(0,0,0,.18)', border: `1px solid ${AC}30` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <span style={{ fontSize: 12, fontWeight: 800, color: '#3f3f46', textTransform: 'uppercase', letterSpacing: '.1em' }}>ETI industrielle 30 M€ · Lyon</span>
      ...
      {[{ l: 'Économies achats an 1', v: '+4 %' }, { l: 'RFQ traités / acheteur / mois', v: '×4' }, { l: 'Fiche comparatif', v: '3 j → 4 h' }, { l: 'Captures an 1', v: '1,2 M€' }].map((r, i) => (
    ...
    // Juridique
    <div style={{ background: '#fff', borderRadius: 14, padding: 20, boxShadow: '0 4px 28px rgba(0,0,0,.09)', border: '1px solid #e4e4e7' }}>
      <div style={{ fontSize: 12, fontWeight: 800, color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 10 }}>Direction juridique ETI · Région</div>
      {[{ l: 'Contrats pré-analysés / an', v: '600' }, { l: 'Jours libérés / mois', v: '4 j' }, { l: 'Délai revue contrat', v: '−75 %' }, { l: 'Souveraineté Mistral', v: '100 % FR' }].map((r, i) => (
        <div key={i} style={{ marginBottom: 10 }}>
          ...
          <div style={{ height: 4, background: '#f4f4f5', borderRadius: 9999, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${[85, 70, 75, 100][i]}%`, background: `linear-gradient(to right,${AC},${AC}88)`, borderRadius: 9999 }} />
          </div>
        </div>
      ))}
    </div>,
...
const metiers = [
  {
    ...
    desc: "Les cabinets d'expertise comptable lyonnais (forte densité Lyon 6ᵉ, 7ᵉ, 9ᵉ et dans la métropole), les directions financières d'ETI régionales et les services compta de scale-up à Confluence absorbent 60 à 80 % de saisie répétitive. Notre cas signature est un cabinet d'expertise comptable lyonnais : ×2 capacité en 4 mois sans recruter, souveraineté Mistral France garantie, 80 nouveaux clients acceptés.",
    pub: 'Cabinets d\'expertise comptable · Directions financières ETI Auvergne-Rhône-Alpes · Compta SaaS B2B Confluence',
    reverse: false,
  },
  {
    n: '02', href: '/agent-ia/ops/', label: 'Opérations et back-office', tag: 'Agent IA Ops →',
    desc: "Les ETI industrielles lyonnaises (Villeurbanne, Bron, Vénissieux, Vallée de la chimie) et les distributeurs B2B régionaux subissent un turnover récurrent sur le poste assistant ops (mails entrants, ADV, gestion documentaire, suivi fournisseurs). Notre agent IA absorbe la couche répétitive et libère vos humains. Cas type Lyon : ETI industrielle qui a triplé son volume traité sans embauche, fin du turnover sur le poste assistant ops.",
    pub: 'ETI industrielles Villeurbanne / Bron / Vénissieux · Distributeurs B2B · ADV et back-office Auvergne-Rhône-Alpes',
    reverse: true,
  },
  {
    n: '03', href: '/agent-ia/achats/', label: 'Achats et sourcing fournisseurs', tag: 'Agent IA Achats →',
    desc: "Les directions achats des ETI manufacturières lyonnaises (sous-traitance industrielle, équipementiers automobile, mécanique de précision) et des laboratoires biotech (Gerland, Lyonbiopôle) gèrent des panels fournisseurs étendus avec des exigences fortes (vigilance financière, conformité ISO, traçabilité). Notre agent IA combine sourcing accéléré, analyse devis pondéré transparent et vigilance financière continue.",
    pub: 'ETI manufacturières Lyon · Biotech Gerland / Lyonbiopôle · Sous-traitance industrielle Auvergne-Rhône-Alpes',
    reverse: false,
  },
  {
    n: '04', href: '/agent-ia/juridique/', label: 'Juridique et conformité', tag: 'Agent IA Juridique →',
    desc: "Les cabinets d'avocats lyonnais (forte densité Part-Dieu et Presqu'Île), les directions juridiques d'ETI régionales et les pharma soumises aux contraintes HAS / ANSM / EMA (Sanofi Pasteur, BioMérieux) gèrent un volume contractuel important et une veille réglementaire dense. Notre agent IA en mode pré-analyse à décharge absorbe la couche répétitive. Souveraineté Mistral hébergé France obligatoire.",
    pub: "Avocats Part-Dieu / Presqu'Île · Directions juridiques ETI · Pharma et biotech Lyon",
    reverse: true,
  },
];

function MetiersLyon() {
    ...
    <section ref={ref} style={{ padding: '96px 24px', background: '#fafafa', borderTop: '1px solid #e4e4e7' }}>
      <div style={{ maxWidth: 1060, margin: '0 auto' }}>
        <div style={{ marginBottom: 56 }}>
          <H2 style={{ marginBottom: 12 }}>L'IA transforme chaque métier lyonnais</H2>
          <p style={{ fontSize: 16, color: '#52525b', maxWidth: 640, lineHeight: 1.7 }}>
            Quatre métiers où nos agents IA ont déjà transformé le quotidien de PME et ETI lyonnaises, du quartier Part-Dieu aux laboratoires Lyonbiopôle de Gerland. Pour la liste complète des 8 métiers couverts, voir notre <a href="/agent-ia/" style={{ color: AC, fontWeight: 700, textDecoration: 'none' }}>hub agents IA par fonction</a>.
          </p>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 56 }}>
          {metiers.map((m, i) => (
            <div key={i} className={`lyo-metier-row${m.reverse ? ' lyo-metier-rev' : ''}`}
              ...
              <div style={{ order: m.reverse ? 2 : 1 }}>
                ...
                <div style={{ padding: '10px 16px', borderRadius: 10, background: '#f0f7ff', border: `1px solid ${AC}15`, marginBottom: 20 }}>
                  <div style={{ fontSize: 10, fontWeight: 800, color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 4 }}>Public type Lyon</div>
              ...
              <div className="lyo-mockup-col" style={{ order: m.reverse ? 1 : 2 }}>
        ...
        <div style={{ marginTop: 64, overflow: 'hidden', borderTop: '1px solid #e4e4e7', paddingTop: 32 }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ display: 'flex', gap: 10, animation: 'lyo-ticker 60s linear infinite', width: 'max-content' }}>
  ...
  return (
    <section ref={ref} style={{ padding: '96px 24px', background: '#fff', borderTop: '1px solid #e4e4e7' }}>
      <div style={{ maxWidth: 1060, margin: '0 auto', opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(24px)', transition: 'all .6s ease' }}>
        <div style={{ fontSize: 12, fontWeight: 800, color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '.14em', marginBottom: 24 }}>Cas client lyonnais · Anonymisé</div>
        <H2 style={{ marginBottom: 20 }}>Cabinet d'expertise comptable lyonnais : ×2 capacité en 4 mois</H2>

        <div style={{ display: 'inline-flex', flexWrap: 'wrap', gap: 6, marginBottom: 40, padding: '6px 14px', borderRadius: 9999, background: '#f4f4f5' }}>
          {['Cabinet d\'expertise comptable', '12 collaborateurs', '320 clients TPE/PME', 'Lyon', '2025-2026', 'cas anonymisé'].map((t) => (
        ...
        <div className="lyo-cas-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1.8fr', gap: 48, alignItems: 'start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              { v: '×2', l: 'capacité gérée à effectif constant', hero: true },
              { v: '80 %', l: 'de la saisie absorbée', hero: false },
              { v: '+80', l: 'nouveaux clients en 4 mois', hero: false },
              { v: '0', l: 'départ d\'équipe après déploiement', hero: false },
            ].map((k, i) => (
          ...
          <div>
            <p style={{ fontSize: 16, color: '#8a8a95', lineHeight: 1.8, marginBottom: 20 }}>
              Le cabinet lyonnais traitait <strong style={{ color: '#09090b' }}>320 clients TPE/PME avec 12 collaborateurs</strong>. La directrice associée refusait de nouveaux clients par peur de la surcharge (2 prospects refusés sur 4). 2 agents IA Althoce déployés en 8 semaines en mode hybride.
            </p>
            <p style={{ fontSize: 16, color: '#8a8a95', lineHeight: 1.8, marginBottom: 28 }}>
              Cadrage présentiel à Lyon, build et déploiement distanciel. Aujourd'hui, le cabinet a accepté <strong style={{ color: '#09090b' }}>80 nouveaux clients sur 4 mois</strong> et recruté 1 expert-comptable junior pour le conseil pur. Souveraineté absolue garantie : Mistral hébergé OVH France, aucune donnée comptable hors territoire.
            </p>
            <div style={{ padding: '20px 24px', borderRadius: 16, background: '#fafafa', border: '1px solid #e4e4e7', borderLeft: `4px solid ${AC}`, marginBottom: 28 }}>
              ...
              <blockquote style={{ fontSize: 16, fontWeight: 700, color: '#09090b', lineHeight: 1.65, margin: '0 0 10px', fontStyle: 'normal' }}>
                On a doublé notre capacité sans recruter. 80 nouveaux clients en 4 mois. La directrice associée peut enfin se concentrer sur le conseil à forte valeur. La modalité hybride proposée par Althoce a parfaitement collé à notre organisation.
              </blockquote>
              <div style={{ fontSize: 13, color: '#8a8a95', fontWeight: 600 }}>Directrice associée · Cabinet d'expertise comptable · Lyon · 2026</div>
            </div>
            <a href="/cas-clients/cabinet-comptable-lyon/" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 15, fontWeight: 700, color: AC, textDecoration: 'none', transition: 'gap .15s' }}
  ...
  return (
    <section id="method" ref={ref} style={{ padding: '96px 24px', background: '#fff', borderTop: '1px solid #e4e4e7' }}>
      <div style={{ maxWidth: 1160, margin: '0 auto' }}>
        ...
        <div className="lyo-grid4" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 24 }}>
          {steps.map((s, i) => (
...
// ── FAQ locale Lyon ───────────────────────────────────────────
const faqLyon = [
  { q: "Présentiel à Lyon ou distanciel : qu'est-ce qui change pour mon projet ?", a: "Rien sur la qualité ni les délais. Nous délivrons exactement les mêmes prestations dans les deux modes. Le présentiel à Lyon (Part-Dieu, Confluence, Presqu'Île, métropole, Auvergne-Rhône-Alpes) est pertinent en phase de cadrage initial et pour les ateliers stratégiques. La phase de build (60 à 70 % du projet) se fait en distanciel structuré avec points hebdomadaires. Vous choisissez la modalité en fonction de votre préférence." },
  { q: "Avez-vous un bureau permanent à Lyon ?", a: "Pas de bureau permanent à Lyon, mais des déplacements réguliers et des espaces partenaires (coworking Part-Dieu ou Confluence selon votre préférence) pour les RDV présentiels. Pour le suivi long terme, le distanciel structuré est notre standard et nos clients lyonnais en sont satisfaits." },
  { q: "Proposez-vous des formations IA pour mes équipes lyonnaises ?", a: "Oui. Nos ateliers de formation IA sont adaptés aux équipes lyonnaises : présentiel dans vos locaux à Lyon ou dans la métropole, ou distanciel synchrone selon votre préférence. Formats : 4h à 21h selon objectifs (utilisation Claude/ChatGPT, bonnes pratiques sécurité, comprendre les agents IA, manager une équipe avec l'IA). Programme adapté par métier." },
  { q: "Mes données restent-elles en France si je travaille avec Althoce à Lyon ?", a: "Oui, par défaut. Mistral hébergé France (OVH Bordeaux, Roubaix, Strasbourg), infrastructure auto-hébergeable. Particulièrement strict pour les clients biotech Gerland, pharma (Sanofi Pasteur, BioMérieux), banque (Crédit Agricole, BPCE) et industrie chimique soumis à des contraintes spécifiques." },
  { q: "Quelle est la différence entre votre approche et celle des cabinets de conseil lyonnais ?", a: "Trois différences. Délai : nous livrons un agent IA en production en 1 semaine, là où un cabinet conseil livre un PowerPoint en plusieurs mois. Livrable : code de production prêt à l'emploi, pas une note d'orientation. Souveraineté France : c'est notre standard par défaut, exigé par beaucoup d'ETI régionales soumises à des contraintes sectorielles fortes." },
  { q: "Avez-vous des clients à Lyon et en Auvergne-Rhône-Alpes ?", a: "Oui, plusieurs. Notre cas client signature finance est un cabinet d'expertise comptable lyonnais. Nous travaillons aussi avec des cabinets d'avocats Part-Dieu, des ETI industrielles régionales, des laboratoires biotech à Gerland, et des éditeurs SaaS B2B à Confluence et Vaise." },
];
    ...
    <section id="faq" style={{ padding: '96px 24px', background: '#fafafa', borderTop: '1px solid #e4e4e7' }}>
      <div style={{ maxWidth: 760, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 52 }}>
          <H2 style={{ marginBottom: 12 }}>Questions fréquentes : Althoce à Lyon et en Auvergne-Rhône-Alpes</H2>
          <p style={{ fontSize: 16, color: '#52525b' }}>Réponses directes aux questions que posent nos prospects lyonnais avant la prise de contact.</p>
        </div>
        <FAQAccordion items={faqLyon} />
      </div>
    </section>
  );
}
...
function Souverainete() {
  ...
  return (
    <section ref={ref} style={{ padding: '96px 24px', background: '#09090b', borderTop: '1px solid #1a1a1a' }}>
      <div style={{ maxWidth: 1160, margin: '0 auto' }}>
        <div className="lyo-split-rev" style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: 64, alignItems: 'center' }}>
    ...
    <section ref={ref} style={{ padding: '96px 24px', background: '#fff', borderTop: '1px solid #e4e4e7' }}>
      <div style={{ maxWidth: 760, margin: '0 auto', textAlign: 'center', opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(24px)', transition: 'all .7s ease' }}>
        ...
        <H2 style={{ marginBottom: 20 }}>
          Prêt à déployer votre premier agent IA<br /><span style={{ color: AC }}>à Lyon ou en Auvergne-Rhône-Alpes ?</span>
        </H2>
        <p style={{ fontSize: 16, color: '#8a8a95', lineHeight: 1.75, marginBottom: 40, maxWidth: 520, margin: '0 auto 40px' }}>
          Un échange de 30 minutes suffit à identifier le cas d'usage le plus impactant. Repartez avec une feuille de route et un devis ferme sous 5 jours. Présentiel à Lyon ou distanciel, que l'on travaille ensemble ou non.
        </p>
...
// ── CSS ───────────────────────────────────────────────────────
const globalStyles = `
  @keyframes lyo-drift { 0%,100%{transform:translate(0,0) scale(1)}50%{transform:translate(3%,4%) scale(1.06)} }
  @keyframes floatCard { 0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)} }
  @keyframes lyo-ticker { from{transform:translateX(0)}to{transform:translateX(-50%)} }

  @media (max-width:860px) {
    .lyo-hero-grid { grid-template-columns:1fr !important; }
    .lyo-map-col { display:none !important; }
    .lyo-split-rev { grid-template-columns:1fr !important; }
    .lyo-cas-grid { grid-template-columns:1fr !important; }
    .lyo-metier-row { grid-template-columns:1fr !important; }
    .lyo-metier-rev > div { order:unset !important; }
    .lyo-mockup-col { display:none !important; }
    .lyo-grid4 { grid-template-columns:repeat(2,1fr) !important; }
  }
  @media (max-width:560px) {
    .lyo-grid4 { grid-template-columns:1fr !important; }
  }
`;

export default function AgenceIALyonPageClient() {
  return (
    <main>
